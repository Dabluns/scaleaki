...
import { BellRing, Smartphone, Zap, Activity, Terminal, LayoutGrid } from 'lucide-react';
import clsx from 'clsx';
import { motion, AnimatePresence } from 'framer-motion';

// ─────────────────────────────────────────────────────────────────
// @ux-design-expert (Uma) · Premium InAppNotifications v5.0
// Design Path: Mastery High-Fidelity / Internal Broadcast Node
// ─────────────────────────────────────────────────────────────────
...
export default function InAppNotifications() {
  ...
  const notificationItems = [
    { key: 'inAppNewOffers', label: 'Offer Broadcast', desc: 'Alertas de contexto interno para novos ativos.', color: 'text-purple-500' },
    { key: 'inAppSimilar', label: 'Pattern Matching', desc: 'Sugestões baseadas em vetores de interesse.', color: 'text-pink-500' },
    { key: 'inAppPerformance', label: 'Real-time Metrics', desc: 'Sinalização de flutuação de dados ativos.', color: 'text-rose-500' },
  ];
      ...
      <div className="relative z-10 flex flex-col gap-12">
        ...
        <header className="flex items-center justify-between border-b border-white/5 pb-10">
          <div className="space-y-2">
            <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter">INTERNAL_BROADCAST</h2>
            <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest italic font-mono">CHANNEL_ID // IN-APP-01</p>
          </div>
          <div className="hidden sm:flex items-center gap-4 px-6 py-2 bg-purple-500/10 border border-purple-500/20 rounded-full">
            <Activity size={14} className="text-purple-500" />
            <span className="text-[9px] font-black text-purple-500 uppercase tracking-widest italic">STREAM_SYNCED</span>
          </div>
        </header>
          ...
          {notificationItems.map((item) => (
            ...
            >
              ...
              <div className={clsx(
                ...
                s[item.key] ? "bg-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.8)] scale-125" : "bg-white/5"
              )} />
            </button>
          ))}
        </div>
        ...
        <div className="space-y-6 pt-10 border-t border-white/5">
          <label className="text-[10px] font-black text-white/20 uppercase tracking-[0.4em] italic flex items-center gap-3">
            <Terminal size={11} className="text-purple-500/40" />
            BROADCAST_BEHAVIOR // FREQUENCY
          </label>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {['realtime', 'daily', 'weekly', 'never'].map((freq) => (
              <button
                key={freq}
                onClick={() => setSettings({ inAppFrequency: freq } as any)}
                className={clsx(
                  "py-4 px-6 rounded-2xl border transition-all duration-700 font-black text-[10px] uppercase tracking-widest italic",
                  (s.inAppFrequency || 'realtime') === freq
                    ? "bg-purple-500 text-black border-purple-500 shadow-2xl scale-105"
